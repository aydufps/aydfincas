...
class AnimalesPesosRouter(Resource):
    def get(self):
        return [i.asJSON() for i in AnimalPeso.query.all()]

    def post(self):
        parser = reqparse.RequestParser()
        parser.add_argument('peso', type=float, required=True,
                            help="El campo peso es obligatorio")
        ...
        item = AnimalPeso(
            peso=args['peso'], animal_id=args['animal_id'], nombre=args["nombre"])
        try:
            db.session.add(item)
            db.session.commit()
...
def animalespesos():
    api.add_resource(AnimalesPesosRouter, '/animales_pesos')
