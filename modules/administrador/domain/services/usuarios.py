...
from modules.shared.infrastructure.repositories.SendEmail import \
...
class Usuarios(Resource):
    ...
    def post(self):
        ...
        parser.add_argument('rol_id', type=str, required=True,
                            help="El campo rol_id es obligatorio")
        ...
        rol_id = args['rol_id']
        # clave = args['clave']
        clave = "0000" if args["clave"] is None else args["clave"]

        usuario = Usuario(name=name, email=email, rol_id=rol_id, clave=clave)
        try:
            db.session.add(usuario)
            db.session.commit()
            enviarEmailNuevoUsuario(email, name, usuario.id)
